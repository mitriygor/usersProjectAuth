...
func (h AuthHandler) NotImplementedHandler(w http.ResponseWriter, r *http.Request) {
	writeResponse(w, http.StatusOK, "Handler is not implemented")
}

func (h AuthHandler) Login(w http.ResponseWriter, r *http.Request) {
	var loginRequest dto.LoginRequest
	if err := json.NewDecoder(r.Body).Decode(&loginRequest); err != nil {
		logger.Error("Error during login request: " + err.Error())
...
func (h AuthHandler) Verify(w http.ResponseWriter, r *http.Request) {
	...
	if urlParams["token"] != "" {
		...
		writeResponse(w, http.StatusForbidden, notAuthorizedResponse("Error missing token"))
	}
}
...
func (h AuthHandler) Refresh(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.NewDecoder(r.Body).Decode(&refreshRequest); err != nil {
		logger.Error("Error during refresh token request: " + err.Error())
		w.WriteHeader(http.StatusBadRequest)
	} else {
